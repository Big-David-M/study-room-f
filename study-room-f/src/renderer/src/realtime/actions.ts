/**
 * Realtime Jeff — voice ACTION tools (card rt-5, Phase 2).
 *
 * The write-side function-tools that turn voice-Jeff into an orchestrator: ping
...
export function realtimeActionTools(): ReturnType<typeof tool>[] {
  return [
    ...
    tool({
      name: 'ping_agent',
      description:
        'Send a short message to one agent (a nudge or note). Soft action — runs immediately, no confirm. Use for "tell Abed X" or "check in with Troy".',
      parameters: {
        type: 'object',
        properties: {
    ...
    tool({
      name: 'dispatch_agent',
      description:
        'Give an agent a task as a structured 4-part work order (objective, context, constraints, done-when) delivered to their inbox. Soft action — runs immediately. Use for "have Troy build X" or "ask Abed to investigate Y".',
      parameters: {
        type: 'object',
        properties: {
    ...
    tool({
      name: 'steer_agent',
      description:
        'Inject live guidance into a running agent to redirect it without stopping it. Soft action — runs immediately. This is the priority verb: "tell Troy to focus on the bug first", "steer Abed away from that approach".',
      parameters: {
        type: 'object',
        properties: {
    ...
    tool({
      name: 'gate_tool',
      description:
        'Block (gate) or unblock one named tool for one agent — e.g. gate Bash for Troy. Soft action — runs immediately.',
      parameters: {
        type: 'object',
        properties: {
          agentId: { type: 'string', description: 'Agent name or id.' },
