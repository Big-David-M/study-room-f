/**
 * Realtime Jeff — renderer voice session (card rt-2, Phase 1 = READ-ONLY voice).
 *
 ...
 * get_config / get_memory / get_activity) and god's rt-6 "Jeff" persona, so the
...
/** Warm openers Jeff leads with the moment a voice session connects, so he
...
const GREETINGS = [
  ...
  "Hey there, Jeff here — what can I do for you?",
...
/** Jeff's voice persona (rt-6 — the final Phase-1 instructions, authored by god). Jeff
 *  is READ-ONLY: he reports on the hive via the rt-4 read-tools but takes no actions yet. */
const MICHAEL_PERSONA =
  `You are Jeff — the voice of the orchestrator ("god") of a hive of autonomous Claude coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
HIVE VOCABULARY. Agents have an id like "abed-mqp3l5wn" and a friendly name like "Abed"; refer to them by name. "god" is the orchestrator whose voice you are. A card's status is todo, doing, blocked, or done. The circuit breaker is healthy, or steering an agent that's looping or idle. Blocked usually means waiting on the human.
...
/** The <audio> sink for Jeff's voice. */
...
function micFriendly(msg: string): string {
  ...
  if (m.includes('permission') || m.includes('notallowed') || m.includes('denied'))
    return 'microphone permission denied — allow mic access to talk to Jeff';
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    // Our own <audio> sink for Jeff's voice, routed to the chosen speaker (rt-8).
    audioEl = new Audio();
    ...
    // Warm-start: a short, best-effort hive snapshot so Jeff's first answer is grounded
    ...
    // rt-12: catch up on completions that finished while no session was open, so Jeff
    ...
    // Floor deltas — silent appends that keep Jeff's picture live without
    ...
    // subscribe so a detected completion makes Jeff speak it unprompted.
    ...
    // Open the conversation: have Jeff speak a warm greeting as his first turn
...
/**
 * React binding for the Realtime Jeff voice loop. Returns the current state plus
