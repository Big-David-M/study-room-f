...
## The cast is *not* LimeZu art (Greendale re-skin)
...
`maps/office.tmj`, `maps/brooklyn99.tmj` and `maps/community.tmj` are Tiled JSON maps built on the LimeZu tilesets
...
`maps/community.tmj` is generated by `tools/gen-community-map.cjs`, which copies furniture stamps
out of `maps/office.tmj`. It therefore draws only LimeZu tiles already bundled here, and the LimeZu
credit obligation above applies to this fork unchanged.

The Greendale cast is drawn by the same procedural recipes as upstream (hair style, skin tone,
garment colours). No artwork, sprites or stills from NBC's *Community* are included, and the
break-room lines in `scene/office/cafeteriaLines.ts` are original writing. This fork is an
affectionate parody and is not affiliated with *Community*, NBC, Sony Pictures Television, or
the upstream Munder Difflin project.
