// Procedural portraits for the Greendale (Community) cast.
//
...
// Tall swept-up quiff: a short cut whose front lifts two rows above the crown
// and leans to one side, so the silhouette reads as deliberately styled.
const styleQuiff: HairFn = (buf, color, skinBase, a) => {
  const [hi, base, sh] = shades(color);
  styleShort(buf, color, skinBase, { part: a.part ?? 'R' });
  for (let x = 6; x <= 12; x++) set(buf, x, 1, base);
  for (let x = 8; x <= 12; x++) set(buf, x, 0, base);
  for (let x = 8; x <= 11; x++) set(buf, x, 0, hi);
  set(buf, 6, 1, sh); set(buf, 12, 1, sh);
};

const HAIR_FNS = { styleShort, styleFloppy, styleFrame, styleBun, styleCurly, styleMessy, styleRecede, styleSpiky, styleBald, styleQuiff };
type HairStyle = keyof typeof HAIR_FNS;

// ─── facial hair ─────────────────────────────────────────────────────────────
type Facial = 'mustache' | 'mustacheSm' | 'stubble' | 'goatee' | 'sideburns';
function drawFacial(buf: Buf, kind: Facial, color: RGB): void {
  ...
  if (kind === 'mustache') {
    ...
    // Long chops down each cheek, flaring outward at the jaw.
    for (let y = 9; y <= 14; y++) { set(buf, HX0, y, base); set(buf, HX1, y, base); }
    for (const y of [11, 12, 13]) { set(buf, HX0 + 1, y, base); set(buf, HX1 - 1, y, base); }
    set(buf, HX0 - 1, 12, base); set(buf, HX1 + 1, 12, base);
  } else if (kind === 'goatee') {
...
const RECIPES: Record<OfficeCharacterName, Recipe> = {
  // ── the study group ──
  jeff:      { skin: 'light', hairc: [150, 96, 52],   hair: 'styleQuiff',  hairargs: { part: 'R' }, cloth: 'suit', c1: [44, 62, 96], pants: [58, 60, 72], facial: 'stubble', brow: 'raised', mouth: 'smile' },
  britta:    { skin: 'light', hairc: [236, 208, 120], hair: 'styleFrame',  hairargs: { length: 18, vol: 1 }, cloth: 'cardigan', c1: [74, 48, 110], c2: [200, 196, 206], brow: 'flat', mouth: 'neutral', lashes: true },
  abed:      { skin: 'tan',   hairc: [26, 22, 24],    hair: 'styleShort',  hairargs: { part: 'R' }, cloth: 'cardigan', c1: [60, 146, 136], c2: [226, 222, 210], brow: 'flat', mouth: 'neutral' },
  troy:      { skin: 'brown', hairc: [24, 20, 20],    hair: 'styleShort',  hairargs: { part: 'L' }, cloth: 'sweater', c1: [84, 88, 100], brow: 'raised', mouth: 'grin' },
  annie:     { skin: 'light', hairc: [52, 34, 28],    hair: 'styleFrame',  hairargs: { length: 16, vol: 1 }, cloth: 'cardigan', c1: [192, 80, 77], c2: [244, 242, 238], brow: 'soft', mouth: 'smile', blush: true, lashes: true },
  shirley:   { skin: 'brown', hairc: [28, 22, 24],    hair: 'styleCurly',  cloth: 'blouse', c1: [142, 74, 158], brow: 'soft', mouth: 'smile', lashes: true, heavy: true },
  pierce:    { skin: 'light', hairc: [222, 220, 214], hair: 'styleBald',   cloth: 'sweater', c1: [140, 52, 56], glasses: true, brow: 'flat', mouth: 'grin' },
  // ── faculty & staff ──
  dean:      { skin: 'light', hairc: [96, 78, 60],    hair: 'styleBald',   hairargs: { recede: 1 }, cloth: 'dressshirt', c1: [150, 186, 226], tie: [196, 84, 140], glasses: true, brow: 'raised', mouth: 'grin' },
  chang:     { skin: 'light', hairc: [22, 20, 24],    hair: 'styleSpiky',  cloth: 'polo', c1: [206, 180, 70], c2: [170, 140, 50], facial: 'goatee', brow: 'angry', mouth: 'grin' },
  duncan:    { skin: 'light', hairc: [126, 96, 62],   hair: 'styleRecede', cloth: 'suit', c1: [110, 92, 66], tie: [70, 90, 80], glasses: true, brow: 'soft', mouth: 'neutral' },
  hickey:    { skin: 'light', hairc: [150, 150, 146], hair: 'styleShort',  hairargs: { part: 'L', recede: 1 }, cloth: 'dressshirt', c1: [110, 126, 108], facial: 'mustache', brow: 'angry', mouth: 'frown' },
  frankie:   { skin: 'light', hairc: [40, 30, 28],    hair: 'styleFrame',  hairargs: { length: 12, vol: 1 }, cloth: 'suit', c1: [82, 110, 126], brow: 'flat', mouth: 'neutral', lashes: true },
  elroy:     { skin: 'dark',  hairc: [160, 156, 150], hair: 'styleBald',   hairargs: { recede: 1 }, cloth: 'cardigan', c1: [122, 90, 58], c2: [214, 206, 186], glasses: true, facial: 'goatee', brow: 'flat', mouth: 'neutral' },
  // ── around campus ──
  magnitude: { skin: 'brown', hairc: [22, 20, 22],    hair: 'styleShort',  hairargs: { part: 'R' }, cloth: 'polo', c1: [222, 172, 58], c2: [180, 130, 40], brow: 'raised', mouth: 'grin' },
  starburns: { skin: 'light', hairc: [46, 36, 30],    hair: 'styleMessy',  hairargs: { length: 8 }, cloth: 'dressshirt', c1: [58, 58, 68], facial: 'sideburns', brow: 'flat', mouth: 'neutral' },
  leonard:   { skin: 'light', hairc: [230, 228, 222], hair: 'styleRecede', cloth: 'dressshirt', c1: [176, 170, 140], brow: 'angry', mouth: 'frown' },
  garrett:   { skin: 'light', hairc: [112, 78, 46],   hair: 'styleCurly',  cloth: 'polo', c1: [112, 156, 92], c2: [80, 120, 66], glasses: true, brow: 'raised', mouth: 'frown' },
};
...
function getBuf(name: OfficeCharacterName): Buf {
  ...
  if (!buf) {
    buf = compose(RECIPES[name] ?? RECIPES.abed);
...
export function sceneFrameBufs(name: OfficeCharacterName): SceneFrames {
  ...
  if (!frames) {
    const r = RECIPES[name] ?? RECIPES.abed;
    frames = {
