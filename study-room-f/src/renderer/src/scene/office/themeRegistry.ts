...
import communityMapRaw from '@/assets/maps/community.tmj?raw';
...
/** Community — Greendale's Study Room F (this fork's default floor).
 *  The map (community.tmj, authored by tools/gen-community-map.cjs) puts the
 *  big study table in the middle of the room: the armchair at its head is
 *  `desk-ceo` (the orchestrator), the eight chairs around it are the next seats
 *  claimed, then the library computer carrels along the north and south walls,
 *  then the two desks in the Dean's office. The right-hand third (cafeteria +
 *  Dean's office) is copied tile-for-tile from office.tmj, so the coffee
 *  economy and the café errand anchors reuse the office coordinates verbatim.
 *  Every stand tile below is asserted reachable by the map generator — if you
 *  move one, mirror it in the generator's STANDS table and re-run it. */
export const COMMUNITY_THEME: ThemeConfig = {
  id: 'community',
  mapRaw: communityMapRaw,
  tilesets: OFFICE_THEME.tilesets,
  primarySeatNames: [
    'desk-ceo',                                                       // head of the study table
    'desk-table-1', 'desk-table-2', 'desk-table-3', 'desk-table-4',   // north side (facing the room)
    'desk-table-5', 'desk-table-6', 'desk-table-7', 'desk-table-8',   // south side
    'pc-1', 'pc-2', 'pc-3', 'pc-4', 'pc-5',                           // north-wall carrels
    'pc-6', 'pc-7', 'pc-8', 'pc-9',                                   // south-wall carrels
    'desk-dean', 'desk-chang',                                        // the Dean's office
  ],
  cafeSeatNames: OFFICE_THEME.cafeSeatNames,
  cafeStands: OFFICE_THEME.cafeStands,
  coffee: OFFICE_THEME.coffee,           // cafeteria sits at the office's coordinates
  anchors: {
    calendar: { x: 4, y: 1 },            // north wall, by the head window → TRIGGERS
    boards: { x: 10, y: 1 },             // whiteboards over the carrels → TASKS
    clock: { x: 1, y: 1 },               // top-left corner → CLOSING TIME
  },
  errandSpots: [
    // floor plants either side of the door, and the cafeteria one
    { kind: 'water', stand: { x: 2, y: 20 }, facing: 'left', fx: { x: 1, y: 20 }, duration: 4.5 },
    { kind: 'water', stand: { x: 22, y: 20 }, facing: 'right', fx: { x: 23, y: 20 }, duration: 4.5 },
    { kind: 'water', stand: { x: 30, y: 20 }, facing: 'right', fx: { x: 31, y: 20 }, duration: 4.5 },
    // the orchestrator's own: the plant on the study table + the corner window
    { kind: 'water', stand: { x: 8, y: 12 }, facing: 'right', fx: { x: 9, y: 11 }, duration: 4.5, godOnly: true },
    { kind: 'smoke', stand: { x: 2, y: 3 }, facing: 'up', fx: { x: 2, y: 1 }, duration: 18, godOnly: true },
    // the two public windows between the carrels
    { kind: 'window', stand: { x: 8, y: 3 }, facing: 'up', fx: { x: 8, y: 1 }, duration: 5 },
    { kind: 'window', stand: { x: 20, y: 3 }, facing: 'up', fx: { x: 20, y: 1 }, duration: 5 },
    // water dispenser in the Dean's office corner
    { kind: 'dispenser', stand: { x: 32, y: 4 }, facing: 'up', fx: { x: 32, y: 3 }, duration: 3.5 },
    // cafeteria fridge + shelf
    { kind: 'fridge', stand: { x: 29, y: 20 }, facing: 'up', fx: { x: 29, y: 19 }, duration: 3.2 },
    { kind: 'shelf', stand: { x: 30, y: 20 }, facing: 'up', fx: { x: 30, y: 18 }, duration: 4 },
    // bins: by the study-room door + the cafeteria
    { kind: 'bin', stand: { x: 18, y: 20 }, facing: 'left', fx: { x: 17, y: 20 }, duration: 2.6 },
    { kind: 'bin', stand: { x: 31, y: 16 }, facing: 'right', fx: { x: 32, y: 16 }, duration: 2.6 },
  ],
  monitor: OFFICE_THEME.monitor,
...
/** Look up a theme by id, falling back to the study room if unknown/missing
 *  (a bad/absent show bundle must never break the floor — see report §E). */
export function getTheme(id: ThemeId): ThemeConfig {
  return THEMES[id] ?? COMMUNITY_THEME;
}
