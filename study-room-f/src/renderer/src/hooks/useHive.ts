...
// The first thing Jeff (god) is told on a fresh spawn — orient him and put
// him to work running the floor. Kept terse and action-oriented.
const INITIAL_GOD_PROMPT = [
  "You're online as Jeff, the orchestrator of the hive. Get oriented, then start running the floor:",
...
function enrichTaskPrompt(text: string): string {
  return [
    ...
    'self-contained prompt to Jeff via an outbox message with "to":"god". Do not do the task yourself.)'
  ].join('\n');
}
 ...
 *   1. spawns the god agent into Jeff's room when none is running,
...
export function useHive(config: HarnessConfig | null): void {
  ...
  // never drains, so the "newest" id was frozen on it: Jeff was nudged once per
  ...
  // Jeff twice (the window between the listPtys check and spawnPty is racy).
  ...
  // Reactive so the assistant bootstrap (effect #1b) re-runs once Jeff is ready.
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const res = await window.cth.spawnPty({
        ...
        cols: 100,
        rows: 30,
        // Restore Jeff's prior conversation across an app restart. His session
      ...
      // Kick Jeff off once his TUI is up. Always re-enable remote control so
      ...
      // a RESUMED Jeff already has his full context and must not be re-oriented
      ...
      // Jeff until he's settled. The live-PTY branch above skips this entirely.
  ...
  useEffect(() => {
    return window.cth.onHiveHookEvent((e) => {
      ...
      if (e.event === 'PreCompact') {
        ...
        // Only (1) is a real "needs you". Treating (2) as blocked made Jeff
  ...
  // 5) Pipe inbound Slack messages into Jeff's queue. The main-process Slack
  //    webhook server pushes each verified message here via IPC; enqueueing to
  //    GOD_ID lands it in Jeff's queue exactly as if the user had typed it
