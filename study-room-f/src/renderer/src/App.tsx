...
export function App() {
  ...
  useEffect(() => {
    ...
    window.cth.getConfig().then(c => {
      ...
      // tvShowOffices flag; off = always the Greendale study room). Settings keeps this synced.
      useStore.getState().setOfficeTheme(c.tvShowOffices ? (c.officeTheme ?? 'community') : 'community');
    ...
    // Realtime Jeff voice toggle can gate on it. Lives in the secret broker, not
  ...
  // hook) so Jeff doesn't boot against the current home while the user may be
  ...
  // (Jeff always spawns, so in normal operation it effectively never runs).
  useEffect(() => {
  ...
  return (
    <div style={{
      ...
      {/* rt-12: global fixed-overlay toast for voice-Jeff completions ("Oscar
