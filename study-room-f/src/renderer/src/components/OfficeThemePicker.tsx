...
const THEME_META: ThemeMeta[] = [
  { id: 'community',     label: 'Community',          blurb: 'Greendale — Study Room F',            built: true,  swatch: '#4f7a5c' },
...
export function OfficeThemePicker({ config }: { config: HarnessConfig }) {
  ...
  const [current, setCurrent] = useState<ThemeId>((config.officeTheme as ThemeId) ?? 'community');
  ...
  const toggleFlag = async () => {
    ...
    try {
      ...
      setOfficeTheme(next ? current : 'community');
    } catch {
