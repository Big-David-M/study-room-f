...
/** Jeff's control surface. Shown instead of the plain terminal/files panel
...
function FloorTab({ seed }: { seed: { text: string; seq: number } }) {
  ...
  // The harness's own default model (Settings → default model). Jeff and every
  ...
  const [dispatchTo, setDispatchTo] = useState<string>(''); // '' = Jeff decides
  ...
  const assignIssue = (issue: GHIssue) => {
    ...
    setDispatchTo(''); // Jeff decomposes and assigns — no more broadcast blasts
  };
