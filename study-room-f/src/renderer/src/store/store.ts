...
export interface Agent {
  ...
  lastPrompt?: string;
  /** the orchestrator ("god") agent — seated in Jeff's room, runs the floor */
  isGod?: boolean;
  /** Jeff's prep assistant — send-only; enriches prompts and forwards them to
   *  the god. Excluded from broadcast fan-out and from the restorable-dead sweep. */
  isAssistant?: boolean;
  /** The human has this agent 1:1 and Jeff has been told to leave it alone.
...
/** Lifecycle of the god agent ("Jeff") bootstrap on launch.
 ...
 *  don't see the "add agent" prompt before Jeff has clocked in. */
...
interface State {
   ...
   *  secret broker, never the store). Gates the Realtime Jeff voice toggle the
...
export const useStore = create<State>((set, get) => ({
  ...
  addAgent: (agent) =>
    set((s) => {
      ...
      // GOD enters at the HEAD, everyone else at the tail. Jeff's position was
