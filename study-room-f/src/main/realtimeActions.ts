/**
 * Realtime Jeff — voice ACTION spine (card rt-5, Phase 2).
 *
 * Phase 1 gave voice-Jeff READ tools. Phase 2 gives him WRITE access: he can
...
const SETTING_POLICY: Record<string, {
  ...
  officeTheme: { tier: 'soft', type: 'string', values: ['community', 'office', 'friends', 'brooklyn99', 'siliconvalley', 'got', 'hogwarts'] },
...
function resolveAgent(target: string, reg: Registry): ResolvedAgent | { error: string } {
  ...
  if ((t === 'god' || t === 'jeff' || t === 'michael' || t === 'the god') && reg.godId)
...
function attribute(deps: RealtimeActionDeps, verb: string, target: string, extra: Record<string, unknown> = {}): void {
  ...
  // rt-7 dual-orchestrator coord: tell the god PTY what voice-Jeff just COMMITTED, so
...
function proposeDestructive(deps: RealtimeActionDeps, verb: string, a: Record<string, unknown>): ActionResult {
  ...
  if (spec.agentTargeted) {
    ...
    // (sessions resume) and "clear Jeff's context" is a real operator need.
