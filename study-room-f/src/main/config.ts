...
export interface HarnessConfig {
  ...
  autoMode: boolean;
  /** May the orchestrator ("Jeff") spin up agents on its own?
   *
  ...
  defaultModel?: string;
  /** Which provider powers the GOD orchestrator ("Jeff"). The persona is
  ...
  /** Master toggle for the Slack → Jeff's-queue integration. */
  ...
  // ─── Realtime Jeff (premium speech-to-speech voice orchestrator) ─────────
  /** True ONLY while a Realtime Jeff voice session is live: the renderer
   ...
   *  tunes this in Settings → Realtime Jeff. */
...
const DEFAULTS: HarnessConfig = {
  ...
  // Community fork: OFF by default. The updater polls the UPSTREAM Munder Difflin
  // releases, and installing one would replace this re-skin with the original.
  autoUpdate: false,
  telemetryEnabled: true,
  multiWindow: true,
  tvShowOffices: false,
  officeTheme: 'community',
  slackEnabled: false,
