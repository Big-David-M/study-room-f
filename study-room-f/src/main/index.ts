...
// Jeff reads + the breaker beat, so guardrails + monitoring work even when the
...
/** Build + write the live fleet snapshot Jeff reads (`<hive>/fleet.json`).
...
// ─── Slack webhook server (Slack message → Jeff's queue) ──────────────────
...
5. REPORT TO GOD — the agent then tells you (Jeff) what it did.
...
function createWindow(opts: { floor?: boolean } = {}): BrowserWindow {
  ...
  const win = new BrowserWindow({
    ...
    title: isFloor ? 'Study Room F — Floor' : 'Study Room F',
  ...
  // Realtime Jeff voice session (`realtimeVoiceEnabled`, flipped on by the
...
async function spawnAgentCore(opts: AgentSpawnOptions, owner: Electron.WebContents | null): Promise<{ ok: boolean; error?: string; cwd?: string; worktreePath?: string; resumeNotFound?: boolean; resumed?: boolean; seedPrompt?: string }> {
  ...
  if (opts.hive && claudeProvider) {
    ...
    // Name the Remote Control session after the agent (Jeff, Jim, Dev1…) so it
...
ipcMain.handle('app:resetAll', () => {
  ...
  // Erase the hive (Jeff's + every agent's memory, inboxes, tasks, board,
...
// (Realtime Jeff's get_agent_detail / list_agents). One read that joins
...
// is live-only) so Jeff can speak to inactive agents — their cwd and memory
...
// ─── IPC: Realtime Jeff (voice orchestrator — ephemeral token mint, rt-1) ──
...
// ─── IPC: Realtime Jeff voice ACTIONS (rt-5, Phase 2) ─────────────────────
...
// ─── IPC: Realtime Jeff completion watcher (rt-12, Phase 2) ───────────────
...
// (so Jeff speaks them unprompted), and bridge waitFor / queue-drain over IPC.
 ...
 *  fleet snapshot Jeff reads (~8s) + the breaker/cost-ledger beat (~30s).
...
app.whenReady().then(() => {
  // Realtime Jeff mic-gate hygiene (rt-8 / Pam rt-10 nit): the voice session
