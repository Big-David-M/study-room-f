...
export interface HiveAgentMeta {
  ...
  cwd: string;
  isGod?: boolean;
  /** Jeff's prep assistant — send-only; enriches prompts and forwards them. */
  isAssistant?: boolean;
}
...
export interface HarnessConfig {
  ...
  defaultModel?: string;
  /** Which provider+model powers the GOD orchestrator ("Jeff"). Default
  ...
  /** Realtime Jeff voice loop — true ONLY while a session holds the mic
   ...
   *  Tuned in Settings → Realtime Jeff; the cost cap stays the runaway guard. */
  ...
  officeTheme?: 'community' | 'office' | 'friends' | 'brooklyn99' | 'siliconvalley' | 'got' | 'hogwarts';
...
const api = {
   ...
   *  keep running; Jeff is told to stop routing work to them. */
   ...
   *  {} for the whole floor. Backs Realtime Jeff's get_messages. The renderer
   ...
   *  archived agents. Backs Realtime Jeff's get_agent_detail / list_agents. */
  ...
  // ─── Slack integration (Slack message → Jeff's queue) ─────────────────────
  ...
  // Realtime Jeff (voice orchestrator) — MAIN mints a short-lived EPHEMERAL token
  ...
  // human-speakable line Jeff relays; the rest is context for a toast/log.
