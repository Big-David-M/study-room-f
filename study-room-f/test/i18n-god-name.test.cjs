...
// "Jeff". The user can rename it, and this codebase has already fixed that
// exact revert three times in the spawn path. It also replaced several
// per-agent runtime names with the literal "Jeff", so the UI named the wrong
...
test('no locale hardcodes the orchestrator name', () => {
  for (const l of LOCALES) {
    const bad = Object.entries(flatten(locale(l)))
      .filter(([, v]) => /Jeff/i.test(text(v)))
      .map(([k]) => k);
    assert.deepEqual(bad, [], `${l}.json hardcodes Jeff in: ${bad.join(', ')}`);
  }
});
...
test('strings about ONE agent interpolate {{name}}, not the orchestrator', () => {
  ...
  // translation nit: "This restarts Jeff" on a dialog that restarts Kevin is
