...
async function floor(t, { steer } = {}) {
  ...
  await hive.ensureAgent({ id: 'god-1', name: 'Jeff', provider: 'claude', cwd: home, isGod: true });
...
function snapshot(hive) {
  hive.writeFleetSnapshot({
    ts: Date.now() - 4000,
    agents: [
      { id: 'god-1', name: 'Jeff', role: 'orchestrator', isGod: true, breaker: 'ok', tokens: 812_400, usd: 4.2199, lastActiveSecAgo: 6, inboxBacklog: 2 },
...
// Jeff has to stop routing to it — otherwise the human and the orchestrator
// are driving the same agent at once, which is how a 1:1 turns into a fight
// over the same terminal.

test('a held agent is marked in the roster and Jeff is told to route around it', async (t) => {
  const { hive } = await floor(t);
  snapshot(hive);
...
test('the hold reaches Jeff without waiting for the next fleet snapshot', async (t) => {
  const { home, hive } = await floor(t);
  snapshot(hive);
...
test('the hold survives a restart, because the registry is the record', async (t) => {
  ...
  assert.equal(reg.agents['jim-1'].onHold, true,
    'a hold that evaporated on restart would hand the agent back to Jeff silently');
});
